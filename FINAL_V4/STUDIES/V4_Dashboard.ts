...
AddLabel(showLabels, "DIR " + (if direction == 2 then "STRONG BULL" else if direction == 1 then "BULL"
      ...
      if direction > 0 then Color.GREEN else if direction < 0 then Color.RED else Color.GRAY);

AddLabel(showLabels, "CONF " + confidence,
      if confidence >= 70 then Color.GREEN else if confidence >= 55 then Color.YELLOW else Color.GRAY);

AddLabel(showLabels, "STABLE " + stableScore,
      if stableScore >= 75 then Color.GREEN else if stableScore >= 60 then Color.CYAN
      else if stableScore >= 45 then Color.YELLOW else Color.GRAY);

AddLabel(showLabels, "EARLY " + earlyScore,
      if earlyScore >= 70 then Color.GREEN else if earlyScore >= 50 then Color.YELLOW else Color.GRAY);

# Concatenating a number to a string is the auto-conversion ThinkScript supports.
# A string/number ternary inside the concat would be a type error, so the NaN
# fallback is numeric (0) rather than "--".
AddLabel(showLabels, "ATR% " + Round(if IsNaN(atrPct) then 0 else atrPct, 2),
      if IsNaN(atrPct) then Color.GRAY else if atrPct >= 5 then Color.MAGENTA
      else if atrPct >= 3 then Color.GREEN else if atrPct >= 2 then Color.CYAN
      else if atrPct >= 1 then Color.YELLOW else Color.GRAY);

AddLabel(showLabels, "RVOL " + Round(if IsNaN(rvol) then 0 else rvol, 2),
      if IsNaN(rvol) then Color.GRAY else if rvol >= 2 then Color.GREEN
      else if rvol >= 1.5 then Color.CYAN else if rvol >= 1 then Color.YELLOW else Color.GRAY);

AddLabel(showLabels, "DARVAS " + darvasScan + " / bias " + darvasBias,
      if darvasScan >= 2 then Color.GREEN else if darvasScan == 1 then Color.YELLOW
      else if darvasScan < 0 then Color.RED else Color.GRAY);

AddLabel(showLabels, "SQZ " + sqzHit + (if sqzState == 2 then " ACTIVE" else if sqzState == 1 then " FIRING"
      else if sqzState == -1 then " POST" else ""),
      if sqzState == 1 then Color.GREEN else if sqzState == 2 then Color.VIOLET
      else if sqzState == -1 then Color.CYAN else Color.GRAY);

AddLabel(showLabels, "FLOW " + smartFlow,
      if smartFlow >= 25 then Color.GREEN else if smartFlow <= -25 then Color.RED else Color.GRAY);

AddLabel(showLabels, "REGIME " + (if regime == 1 then "TREND" else if regime == 2 then "BREAKOUT"
      else if regime == 3 then "SQUEEZE" else if regime == 4 then "REVERSAL"
      else if regime == 5 then "HIGH VOL" else if regime == 6 then "LOW VOL" else "CHOP"),
      if regime == 3 then Color.VIOLET else if regime == 2 then Color.CYAN
      else if regime == 1 then Color.GREEN else if regime == 4 then Color.ORANGE
      else if regime == 5 then Color.RED else Color.GRAY);

AddLabel(showLabels, "TIMING " + (if v4Timing == 2 then "PRIME" else if v4Timing == 1 then "EARLY"
      else if v4Timing == -1 then "EXTENDED" else if v4Timing == -2 then "EXHAUSTED" else "NEUTRAL"),
      if v4Timing >= 1 then Color.GREEN else if v4Timing <= -1 then Color.RED else Color.GRAY);

AddLabel(showLabels, "EXT " + Round(if IsNaN(extensionATR) then 0 else extensionATR, 2) + " ATR",
      if IsNaN(extensionATR) then Color.GRAY else if extensionATR > 2 then Color.RED
      else if extensionATR > 1.5 then Color.ORANGE else if extensionATR > 1 then Color.YELLOW else Color.GRAY);

AddLabel(showLabels, "LIQ " + liquidityScore,
      if liquidityScore >= 65 then Color.GREEN else if liquidityScore >= 50 then Color.CYAN
      else if liquidityScore >= 35 then Color.YELLOW else Color.RED);

AddLabel(showLabels, "CONFIRM " + v4Confirmed + "/4",
      if v4Confirmed >= 4 then Color.GREEN else if v4Confirmed == 3 then Color.CYAN
      else if v4Confirmed == 2 then Color.YELLOW else Color.GRAY);

plot Zero = Double.NaN;
Zero.SetDefaultColor(Color.CURRENT);
Zero.HideBubble();
Zero.HideTitle();
