...
# V4_DarvasBox.ts
# ==========================================================================
# PURPOSE      : Draw the Darvas box the scanner actually uses, so you can eyeball it.
# AGGREGATION  : Any.
# INSTALL      : Charts > Studies > Edit Studies > Create > paste > apply to the chart.
# OUTPUT       : Box top and bottom plotted as step lines, plus breakout arrows.
# COLORS       : Green top, red bottom, arrows on state changes.
# LIMITATIONS  : Uses high[1]/low[1] so the box does not include the forming bar. This is why the plotted box can sit slightly below an intrabar spike.
# ==========================================================================
...
BoxTop.SetStyle(Curve.SHORT_DASH);

plot BoxBot = boxBot;
BoxBot.SetDefaultColor(Color.RED);
BoxBot.SetStyle(Curve.SHORT_DASH);

plot BreakUp = if darvasScan >= 2 and darvasScan[1] < 2 then low - atr * 0.5 else Double.NaN;
BreakUp.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
BreakUp.SetDefaultColor(Color.GREEN);

plot BreakDn = if darvasScan <= -1 and darvasScan[1] > -1 then high + atr * 0.5 else Double.NaN;
BreakDn.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
