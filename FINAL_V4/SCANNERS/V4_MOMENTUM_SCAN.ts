...
# V4_MOMENTUM_SCAN.ts
# ==========================================================================
# PURPOSE      : Active, clean momentum that is already working.
...
# OUTPUT       : true = symbol passes. Trending, efficient, expanding momentum with directional agreement.
...
input minADX     = 22;
input minEffRatio = 0.35;
input minConf    = 60;

plot scan = adx >= minADX
        and effRatio >= minEffRatio
        and confidence >= minConf
        and AbsValue(direction) >= 1
        and AbsValue(sqzMom) > AbsValue(sqzMom[3])
        and ((direction > 0 and stackUp >= 2) or (direction < 0 and stackDn >= 2))
