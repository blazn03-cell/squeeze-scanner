# ==========================================================================
# V4_SQUEEZE_FIRE_SCAN.ts
# ==========================================================================
# PURPOSE      : Squeezes that just released, with momentum and volume behind them.
...
# OUTPUT       : true = symbol passes. Fired within maxBarsSinceFire bars and still expanding.
# COLORS       : n/a — scan filters have no colour output.
# LIMITATIONS  : Exactly one plot, no labels, no bubbles, no secondary aggregation. Set the scan aggregation in the Study Filter's own dropdown and keep it identical to your watchlist columns or the numbers will not match. 
# ==========================================================================
...
input minSqueezeHit     = 60;
input maxBarsSinceFire  = 3;
input minRvol           = 1.2;

plot scan = sinceFire <= maxBarsSinceFire
        and !sqzOn
        and sqzHit >= minSqueezeHit
        and rvolSafe >= minRvol
        and AbsValue(direction) >= 1
        and AbsValue(sqzMom) > AbsValue(sqzMom[1])
