...
# V3_StableScore.ts
# ==========================================================================
# PURPOSE      : 0-100 setup-quality score across six correlation-separated buckets.
# AGGREGATION  : Daily for swing, 15m for intraday. Keep it consistent with the scans.
# INSTALL      : MarketWatch > Quotes > Customize > Custom Quotes > new > paste.
# OUTPUT       : 0-39 weak · 40-54 below avg · 55-64 acceptable · 65-74 strong · 75-84 very strong · 85-100 elite. Elite requires 3+ buckets at 70%.
# COLORS       : grey/red weak · amber acceptable · teal strong · green elite.
# LIMITATIONS  : Needs ~100 bars for the Bollinger-width percentile to be meaningful. Direction-agnostic by design: a strong short scores the same as a strong long.
# ==========================================================================
...
plot StableScore = stableScore;
StableScore.SetDefaultColor(Color.WHITE);
AssignBackgroundColor(
      if IsNaN(stableScore) then Color.DARK_GRAY
      else if stableScore >= 85 then CreateColor(0, 170, 90)
      else if stableScore >= 75 then CreateColor(0, 130, 80)
      else if stableScore >= 65 then CreateColor(0, 110, 130)
      else if stableScore >= 55 then CreateColor(140, 120, 0)
      else if stableScore >= 40 then CreateColor(90, 60, 30)
      else CreateColor(70, 25, 35));
