...
# DarvasBias.ts
# ==========================================================================
# PURPOSE      : Darvas STRUCTURAL posture — separate from the breakout event.
# AGGREGATION  : Same as DarvasScan — Daily default, 4h fast variant.
# INSTALL      : MarketWatch > Quotes > Customize > Custom Quotes > new > paste.
# OUTPUT       : -2 strong bearish · -1 bearish · 0 neutral · +1 bullish · +2 strong bullish.
# COLORS       : red shades bearish · grey neutral · green shades bullish.
# LIMITATIONS  : Built from box position + box progression + EMA stack, so it can be +2 while DarvasScan is 0. That divergence is the signal, not a bug.
# ==========================================================================
...
plot Bias = darvasBias;
Bias.SetDefaultColor(Color.WHITE);
AssignBackgroundColor(
      if darvasBias == 2 then CreateColor(0, 175, 95)
      else if darvasBias == 1 then CreateColor(0, 110, 70)
      else if darvasBias == -1 then CreateColor(130, 40, 55)
      else if darvasBias == -2 then CreateColor(185, 30, 55)
