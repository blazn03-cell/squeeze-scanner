...
async function analyzeFile(client, filename, source) {
  ...
  catch { return JSON.parse(text.replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/, "").trim()); }
}

function projectFiles(root) {
  return readdirSync(root)
    .filter(f => f.endsWith(".js") && f !== "improve-agent.js" && !f.startsWith("."))
    .map(f => resolve(root, f));
}

const RESET="\x1b[0m",BOLD="\x1b[1m",DIM="\x1b[2m";
const severityColor=sev=>({high:"\x1b[31m",medium:"\x1b[33m",low:"\x1b[36m"})[sev]??"";

function printReport(result) {
  if (!result?.file) return;
  console.log(`\n${BOLD}━━━ ${result.file} ━━━${RESET}`);
  const issues = result.issues || [];
  if (!issues.length) { console.log(`  ${DIM}No issues found.${RESET}`); return; }
...
async function main() {
  ...
  const targets = process.argv.slice(2).length > 0
    ...
    : projectFiles(root);
  if (!targets.length) { console.error("No JS files found to review."); process.exit(1); }
  ...
  for (const filePath of targets) {
    ...
    catch { console.warn(`  Skipping ${filename} — could not read file.`); continue; }
    process.stdout.write(`  Analyzing ${filename}…`);
    let result;
    try { result = await analyzeFile(client, filename, source); }
    catch (err) { console.log(` failed (${err?.message||err})`); continue; }
  ...
  console.log(`\n${BOLD}━━━ Summary ━━━${RESET}`);
  summary.forEach(({file,count}) => console.log(`  ${file}: ${count} issue${count===1?"":"s"}`));
  console.log(`  Total: ${totalIssues} issue${totalIssues===1?"":"s"} across ${targets.length} file${targets.length===1?"":"s"}`);
}

main().catch(err => { console.error("Agent error:", err?.message||err); process.exit(1); });
